...
public abstract class ErasureDecoder extends Configured
  ...
  public ErasureDecoder(ErasureCoderOptions options) {
    ...
    this.numLocalParityUnits = options.getNumLocalParityUnits();
  }
  ...
  public int getNumLocalParityUnits() {
    return this.numLocalParityUnits;
  }
  ...
  protected ECBlock[] getInputBlocks(ECBlockGroup blockGroup) {
    ECBlock[] inputBlocks = new ECBlock[getNumDataUnits() +
            getNumParityUnits() + getNumLocalParityUnits()];
    ...
    System.arraycopy(blockGroup.getLocalParityBlocks(), 0, inputBlocks,
            getNumDataUnits(), getNumLocalParityUnits());
  ...
  protected ECBlock[] getOutputBlocks(ECBlockGroup blockGroup) {
    ...
    for (int i = 0; i < getNumLocalParityUnits(); i++) {
      if (blockGroup.getLocalParityBlocks()[i].isErased()) {
        outputBlocks[idx++] = blockGroup.getLocalParityBlocks()[i];
      }
    }
  ...
  protected int getNumErasedBlocks(ECBlockGroup blockGroup) {
    ...
    num += getNumErasedBlocks(blockGroup.getLocalParityBlocks());
    return num;
  }
