...
public class ECBlockGroup {
  ...
  private ECBlock[] localParityBlocks;
   ...
   * @param dataBlocks data blocks in the group
   * @param parityBlocks parity blocks in the group
   *  @param localParityBlocks local parity blocks in the group
   */
  public ECBlockGroup(ECBlock[] dataBlocks, ECBlock[] parityBlocks, ECBlock[] localParityBlocks) {
    this.dataBlocks = dataBlocks;
    this.parityBlocks = parityBlocks;
    this.localParityBlocks=localParityBlocks;
  }
  /**
   * Get data blocks
   * @return data blocks
  ...
  /**
   * Get local parity blocks
   * @return local parity blocks
   */
  public ECBlock[] getLocalParityBlocks() {
    return localParityBlocks;
  }
  /**
  ...
  /**
   * Any erased local parity block?
   * @return true if any erased local parity block, false otherwise
   */
  public boolean anyErasedLocalParityBlock() {
    for (int i = 0; i < localParityBlocks.length; ++i) {
      if (localParityBlocks[i].isErased()) {
        return true;
      }
    }
  ...
  public int getErasedCount() {
    ...
    for (ECBlock localparityBlock : localParityBlocks) {
      if (localparityBlock.isErased()) erasedCount++;
    }
    return erasedCount;
  }
