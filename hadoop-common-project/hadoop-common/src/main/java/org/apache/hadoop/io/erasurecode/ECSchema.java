...
public final class ECSchema implements Serializable {
  ...
  public static final String NUM_LOCAL_PARITY_UNITS_KEY = "numLocalParityUnits";
  ...
  /**
   * Number of local parity units generated in a coding
   */
  private final int numLocalParityUnits;
  ...
  public ECSchema(Map<String, String> allOptions) {
    ...
    int tmpNumDataUnits;
    int tmpNumLocalParityUnits;
    int tmpNumParityUnits;
    if(codecName == "lrc"){
      tmpNumDataUnits = extractIntOption(NUM_DATA_UNITS_KEY, allOptions);
      tmpNumLocalParityUnits = extractIntOption(NUM_LOCAL_PARITY_UNITS_KEY, allOptions);
      tmpNumParityUnits = extractIntOption(NUM_PARITY_UNITS_KEY, allOptions);
      if (tmpNumDataUnits < 0 || tmpNumParityUnits < 0 || tmpNumLocalParityUnits < 0) {
        throw new IllegalArgumentException(
            "No good option for numDataUnits or numParityUnits or numLocalParityUnits found ");
      }
    }else {
      tmpNumDataUnits = extractIntOption(NUM_DATA_UNITS_KEY, allOptions);
      tmpNumLocalParityUnits = 0;
      tmpNumParityUnits = extractIntOption(NUM_PARITY_UNITS_KEY, allOptions);
    ...
    this.numLocalParityUnits = tmpNumLocalParityUnits;
    ...
    allOptions.remove(NUM_LOCAL_PARITY_UNITS_KEY);
   ...
   * @param codecName codec name
   * @param numDataUnits number of data units used in the schema
   * @param numLocalParityUnits number of local parity units used in the schema
   * @param numParityUnits number os parity units used in the schema
   */
  public ECSchema(String codecName, int numDataUnits, int numLocalParityUnits, int numParityUnits) {
   ...
   * @param codecName codec name
   * @param numDataUnits number of data units used in the schema
   * @param numLocalParityUnits number of local parity units in the schema
   * @param numParityUnits number os parity units used in the schema
   * @param extraOptions extra options to configure the codec
   */
  public ECSchema(String codecName, int numDataUnits, int numLocalParityUnits, int numParityUnits,
                  Map<String, String> extraOptions) {
    assert (codecName != null && ! codecName.isEmpty());
    assert (numDataUnits > 0 && numParityUnits > 0 && numLocalParityUnits > 0);
  ...
  /**
   * Get required local parity units count in a coding group
  ...
  public String toString() {
    ...
    if(codecName != "lrc"){
      ...
      sb.append("Codec=" + codecName + ", ");
      sb.append(NUM_DATA_UNITS_KEY + "=" + numDataUnits + ", ");
      sb.append(NUM_LOCAL_PARITY_UNITS_KEY+ "=" + numLocalParityUnits + ", ");
      sb.append(NUM_PARITY_UNITS_KEY + "=" + numParityUnits);
      sb.append((extraOptions.isEmpty() ? "" : ", "));
    }
  ...
  public boolean equals(Object o) {
    ...
    if(codecName != "lrc"){
      ...
      return new EqualsBuilder()
          ...
          .append(extraOptions, rhs.extraOptions)
          .append(numDataUnits, rhs.numDataUnits)
          .append(numLocalParityUnits, rhs.numLocalParityUnits)
          .append(numParityUnits, rhs.numParityUnits)
          .isEquals();
    }
  }
