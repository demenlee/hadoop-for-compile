...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
  ...
  public DatanodeStorageInfo[] chooseTarget(String srcPath,
    ...
    LOG.warn("chosennodes is {}", chosenNodes.size());
  ...
  DatanodeStorageInfo[] chooseTarget(String src,
    ...
    try {
      ...
      LOG.warn("the size of results is {}", results.size());
      if (results.size() < numOfReplicas) {
        ...
        DatanodeStorageInfo[] remainingTargets =
            chooseTarget(src, numOfReplicas, writer,
                ...
                storageTypes);
        LOG.warn("the length of remaining targets is {}",remainingTargets.length);
  ...
  private DatanodeStorageInfo[] chooseTarget(int numOfReplicas,
    ...
    if (numOfReplicas == 0 || clusterMap.getNumOfLeaves()==0) {//决策树？
    ...
    for(int i=0;i<result.length;++i){
      LOG.warn("the results {} is {}", i ,result[i]);
    }
    ...
    LOG.warn("the length of results is {}" ,results.size());
    // sorting nodes to form a pipeline
    return getPipeline((writer != null && writer instanceof DatanodeDescriptor) ? writer
            : localNode, results.toArray(new DatanodeStorageInfo[results.size()]));
  }
