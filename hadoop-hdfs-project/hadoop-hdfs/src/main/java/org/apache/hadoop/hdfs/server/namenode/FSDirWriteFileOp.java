...
class FSDirWriteFileOp {
  public static final org.slf4j.Logger LOG = LoggerFactory
      .getLogger(FSDirWriteFileOp.class.getName());
    ...
    if (blockType == BlockType.STRIPED) {
      ...
      numTargets = (short) (ecPolicy.getSchema().getNumDataUnits()
          + ecPolicy.getSchema().getNumParityUnits()
              + ecPolicy.getSchema().getNumLocalParityUnits());
    } else {
  ...
  static LocatedBlock storeAllocatedBlock(FSNamesystem fsn, String src,
    ...
    if (onRetryBlock[0] != null) {
      if (onRetryBlock[0].getLocations().length > 0) {
        ...
        LOG.warn("the length of targets is {}", targets.length);
        LOG.warn("the lastBlockInFile is {}", lastBlockInFile.getUnderConstructionFeature().getBlockIndices().length);
  ...
  private static BlockInfo addBlock(FSDirectory fsd, String path,
    ...
    try {
      ...
      if (blockType == BlockType.STRIPED) {
        ...
        short numLocalParityUnits = (short) ecPolicy.getNumLocalParityUnits();
        short numParityUnits = (short) ecPolicy.getNumParityUnits();
        short numLocations = (short) (numDataUnits + numParityUnits + numLocalParityUnits);
