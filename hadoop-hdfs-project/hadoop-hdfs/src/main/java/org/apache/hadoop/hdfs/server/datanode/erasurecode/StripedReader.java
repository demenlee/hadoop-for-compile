...
class StripedReader {
  ...
  private final int localParityBlkNum;
  ...
  StripedReader(StripedReconstructor reconstructor, DataNode datanode,
    ...
    localParityBlkNum = stripedReconInfo.getEcPolicy().getNumLocalParityUnits();
  ...
  private void initZeroStrip() {
    ...
    int k = 0;
    for (int i = 0; i < dataBlkNum + parityBlkNum + localParityBlkNum; i++) {
      if (!bitset.get(i)) {
  ...
  ByteBuffer[] getInputBuffers(int toReconstructLen) {
    ByteBuffer[] inputs = new ByteBuffer[dataBlkNum + parityBlkNum + localParityBlkNum];
