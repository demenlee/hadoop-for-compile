...
abstract class StripeReader {
  ...
  protected final short localParityBlkNum;
  protected final short parityBlkNum;
  protected final short allBlkNum;
  ...
  StripeReader(AlignedStripe alignedStripe,
    ...
    this.localParityBlkNum = (short)ecPolicy.getNumLocalParityUnits();
    this.parityBlkNum = (short)ecPolicy.getNumParityUnits();
    this.allBlkNum = (short) (dataBlkNum + localParityBlkNum + parityBlkNum);
  ...
  private void checkMissingBlocks() throws IOException {
    if(ecPolicy.getCodecName() != "lrc"){
      ...
      if (alignedStripe.missingChunksNum > parityBlkNum + 1) {
        clearFutures();
        throw new IOException(alignedStripe.missingChunksNum
            + " missing blocks, the stripe is: " + alignedStripe
            + "; locatedBlocks is: " + dfsStripedInputStream.getLocatedBlocks());
  ...
  private void readDataForDecoding() throws IOException {
    ...
    if(ecPolicy.getSchema().getCodecName() != "lrc"){
      ...
      for(int i = 0; i < ecPolicy.getNumLocalParityUnits(); i++){
        for(int j = dataBlkNum / localParityBlkNum * i; j < dataBlkNum / localParityBlkNum * (i+1); j++){
          Preconditions.checkNotNull(alignedStripe.chunks[j]);
          if(alignedStripe.chunks[j].state == StripingChunk.REQUESTED){
            if (!readChunk(targetBlocks[j], j)) {
              alignedStripe.missingChunksNum++;
              alignedStripe.missingLocalChunksNum[i]++;
            }
          }
        }
      }
  ...
  void readParityChunks(int num) throws IOException {
    for (int i = dataBlkNum + localParityBlkNum, j = 0; i < dataBlkNum + localParityBlkNum + parityBlkNum && j < num; i++) {
  ...
  void readLocalParityChunks(int[] localnum) throws IOException {
    for(int i = dataBlkNum; i < dataBlkNum + localParityBlkNum; i++){
      if(alignedStripe.chunks[i] == null){
        if(!(prepareParityChunk(i) && readChunk(targetBlocks[i], i))){
          alignedStripe.missingLocalChunksNum[i - dataBlkNum]++;
        }
      }
    }
    checkMissingBlocks();
