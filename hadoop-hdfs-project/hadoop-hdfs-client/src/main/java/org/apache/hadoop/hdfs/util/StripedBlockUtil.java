...
public class StripedBlockUtil {
   ...
   * @param parityBlkNum
   *          The number of parity blocks
   * @param localParityBlkNum
   *          The number of local parity blocks
   * @param cellSize
   *          The size of a striping cell
   * @return The total usage of data blocks and parity blocks
   */
  public static long spaceConsumedByStripedBlock(long numDataBlkBytes,
                                                 int dataBlkNum, int parityBlkNum, int localParityBlkNum, int cellSize) {
    int localparityIndex = dataBlkNum + 1;
    int parityIndex = dataBlkNum + localParityBlkNum + 1;
    long numParityBlkBytes = getInternalBlockLength(numDataBlkBytes, cellSize,
            dataBlkNum, parityIndex) * parityBlkNum;
    long numLocalParityBlkBytes = getInternalBlockLength(numDataBlkBytes, cellSize,
            dataBlkNum, localParityBlkNum) * localParityBlkNum;
    return numDataBlkBytes + numParityBlkBytes + numLocalParityBlkBytes;
  }
  /**
  ...
  private static VerticalRange[] getRangesForInternalBlocks(
    ...
    int localParityBlkNum=ecPolicy.getNumLocalParityUnits();

    VerticalRange[] ranges = new VerticalRange[dataBlkNum + parityBlkNum + localParityBlkNum];
    ...
    for (int i = dataBlkNum; i < dataBlkNum + parityBlkNum + localParityBlkNum; i++) {
    ...
    for (long point : stripePoints) {
      if (prev >= 0) {
        stripes.add(new AlignedStripe(prev, point - prev,
            dataBlkNum + parityBlkNum + localParityBlkNum));
      }
      prev = point;
    }
  ...
  public static class AlignedStripe {
    ...
    public int[] missingLocalChunksNum;
